...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'createdTime' | 'lastUpdate'> & {
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      createdTime: customer.createdTime?.toJSON() ?? null,
      lastUpdate: customer.lastUpdate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...restCustomer,
      createdTime: restCustomer.createdTime ? dayjs(restCustomer.createdTime) : undefined,
      lastUpdate: restCustomer.lastUpdate ? dayjs(restCustomer.lastUpdate) : undefined,
    };
  }
