...
describe('Product Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call ProductCategory query and add missing value', () => {
      ...
      const categorie: IProductCategory = { id: 36172 };
      ...
      const additionalProductCategories = [categorie];
    ...
    it('Should call Productvariante query and add missing value', () => {
      const product: IProduct = { id: 456 };
      const productvariantes: IProductvariante[] = [{ id: 83522 }];
      product.productvariantes = productvariantes;

      const productvarianteCollection: IProductvariante[] = [{ id: 45095 }];
      jest.spyOn(productvarianteService, 'query').mockReturnValue(of(new HttpResponse({ body: productvarianteCollection })));
      const additionalProductvariantes = [...productvariantes];
      const expectedCollection: IProductvariante[] = [...additionalProductvariantes, ...productvarianteCollection];
      jest.spyOn(productvarianteService, 'addProductvarianteToCollectionIfMissing').mockReturnValue(expectedCollection);

      activatedRoute.data = of({ product });
      comp.ngOnInit();

      expect(productvarianteService.query).toHaveBeenCalled();
      expect(productvarianteService.addProductvarianteToCollectionIfMissing).toHaveBeenCalledWith(
        productvarianteCollection,
        ...additionalProductvariantes.map(expect.objectContaining)
      );
      expect(comp.productvariantesSharedCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const product: IProduct = { id: 456 };
      const categorie: IProductCategory = { id: 58801 };
      product.categorie = categorie;
      const productvariante: IProductvariante = { id: 76481 };
      product.productvariantes = [productvariante];

      activatedRoute.data = of({ product });
      comp.ngOnInit();

      expect(comp.productCategoriesSharedCollection).toContain(categorie);
      expect(comp.productvariantesSharedCollection).toContain(productvariante);
  ...
  describe('Compare relationships', () => {
    ...
    describe('compareProductvariante', () => {
      it('Should forward to productvarianteService', () => {
        const entity = { id: 123 };
        const entity2 = { id: 456 };
        jest.spyOn(productvarianteService, 'compareProductvariante');
        comp.compareProductvariante(entity, entity2);
        expect(productvarianteService.compareProductvariante).toHaveBeenCalledWith(entity, entity2);
      });
    });
  });
});
